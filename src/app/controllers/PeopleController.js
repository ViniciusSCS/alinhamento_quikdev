...
class PeopleController {
  ...
  async me (req, res) {
    try {
      ...
      if (!payload) { return sendNotFoundError(req, res, `Usuário ${userId} não encontrado`) }
  ...
  async delete (req, res) {
    ...
    try {
      const peopleExists = await models.people.findOne({ where: { id } })

      if (!peopleExists) {
        return res.status(404).json({ msg: 'Pessoa não encontrada', data: {} })
      } else {
        await peopleExists.destroy({ where: { id } })
        return res
          .status(200)
